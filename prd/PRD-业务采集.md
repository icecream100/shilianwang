...
- 主要原型页面: P_DataColl_Biz_ConfigList (业务采集设置列表页), P_DataColl_Biz_AddEditConfig (新增/编辑业务采集设置页)

#### 4.1.3 详细需求

##### 4.1.3.1 功能入口与触发条件

- 入口: 运营门户模块 -> 数据管理 -> 业务采集设置 [参考原型 P_DataColl_Biz_ConfigList]
...
- **A. 数据采集任务列表与筛选**: [参考原型 P_DataColl_Biz_ConfigList]
    ...
    - 默认交互/排序: 列表默认按创建时间降序。
- **B. 新增/编辑业务采集设置**: [参考原型 P_DataColl_Biz_AddEditConfig]
    - 触发: 点击“新增采集配置”按钮或已有配置的“编辑”按钮。
    - 交互步骤:
        - Step 1: 系统弹出“业务采集设置”对话框/页面。
        - Step 2: 用户输入配置名称。
...
- 入口: 运营门户模块 -> 数据管理 -> 客户同步设置 [参考原型 P_DataColl_CRM_Config]
