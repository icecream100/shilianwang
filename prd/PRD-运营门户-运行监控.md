...
## 1\. 功能概述
  ...
  - 1.1 功能名称：运行监控
  - 1.2 功能概述：运行监控模块为视联网省级适配中心提供实时的状态监测、灵活的告警规则配置以及规范化的告警事件管理能力。它基于适配中心自身健康数据、与省级平台的连接状态数据及关键业务指标数据，通过用户自定义的告警规则，自动生成告警事件，并将告警信息在适配中心内部（如驾驶舱）进行展示。
...
#### 4.1.1 功能描述与目标
  ...
  - 解决的问题: 运维人员需要一种灵活的方式来定义何种情况下系统应发出告警，以主动发现潜在或已发生的问题。
  - 用户价值/目标: 允许管理员根据实际运维需求和业务特点，针对各类监控指标自定义告警触发条件，实现精准、有效的自动化监控预警。
  - [cite\_start]核心能力概述: 提供用户界面，允许管理员或高级运维人员基于已采集的适配中心健康数据、省级平台连接状态数据、关键业务指标数据（如视频流请求成功率、接口调用超时次数）等，灵活配置告警触发规则。规则应支持多条件组合（与/或）、比较操作符（大于/小于/等于）、持续时间阈值（如CPU使用率连续5分钟超过80%）等。 [cite: 1]

#### 4.1.2 原型参考

  - 主要原型页面: P\_Monitor\_Rule\_List (告警规则列表页), P\_Monitor\_Rule\_AddEdit (新增/编辑告警规则页)

#### 4.1.3 详细需求

##### 4.1.3.1 功能入口与触发条件

  - 入口: 运营门户模块 -\> 运行监控 -\> 告警规则配置 [参考原型 P\_Monitor\_Rule\_List]
  - 触发条件: 用户已登录，并拥有“告警规则配置”权限。

##### 4.1.3.2 交互流程与界面详述

  - **A. 告警规则列表与查询**: [参考原型 P\_Monitor\_Rule\_List]
      - 布局: 顶部为“新增规则”按钮和筛选区域，下方为已配置的告警规则列表。
      - 筛选条件: 支持按规则名称、监控对象类型（如：系统资源、平台连接、业务指标）、告警级别、启用状态进行筛选。
      - 规则列表: 表格形式展示。关键列包括：规则名称、监控对象、告警条件简述、告警级别、触发动作（如生成告警、通知）、启用状态、创建人、创建时间、最后修改时间。操作列包含：编辑、删除、查看详情、启用/停用。
      - 默认交互/排序: 列表默认按创建时间降序。
  - **B. 新增/编辑告警规则**: [参考原型 P\_Monitor\_Rule\_AddEdit]
      ...
      - 交互步骤:
          ...
          - Step 6: 设置告警级别：选择告警产生时的级别（如：提示、警告、重要、紧急/P0-P3）。
          - Step 7: 配置告警动作 (基础)：
              - 生成告警事件: 默认勾选。
...
| 告警级别 | alarmLevel | Enum(P0, P1, P2, P3, INFO) | 显示/选择 | 必填                                                                                                      | 用户选择 |  |
...
##### 4.1.3.4 权限控制
  ...
  - 查看/新增/编辑/删除/启停用告警规则: 需要特定权限，如“告警规则管理权限”，通常授予管理员或高级运维人员。
...
#### 4.2.1 功能描述与目标
  ...
  - 解决的问题: 当监控数据满足预设的告警规则时，系统需要自动、实时地生成结构化的告警事件，并提供界面对这些告警进行查看、管理和跟踪处理，同时确保关键告警能被有效传递到相关平台。
...
#### 4.2.2 原型参考
  ...
  - 主要原型页面: P\_Monitor\_Alarm\_List (告警列表页), P\_Monitor\_Alarm\_View (告警详情页), P\_Monitor\_Alarm\_Report\_Config (告警上报配置页 - 若本地可配)

#### 4.2.3 详细需求

##### 4.2.3.1 功能入口与触发条件

  - 入口: 运营门户模块 -\> 运行监控 -\> 告警管理 (或告警中心) [参考原型 P\_Monitor\_Alarm\_List]
  - 触发条件: 用户已登录，并拥有“告警查看”或“告警处理”权限。

##### 4.2.3.2 交互流程与界面详述

  - **A. 告警列表 (活动告警/历史告警)**: [参考原型 P\_Monitor\_Alarm\_List]
      - 布局: 顶部为筛选条件区和操作区（如“全部标为已读”、“批量确认”等），下方为告警事件列表。可提供“当前活动告警”和“历史告警”的标签页切换。
      - 筛选条件:
          - 告警级别: 多选（P0, P1, P2, P3, INFO等）。
          - 告警状态 (本地状态): 多选（如：新告警、已确认/Acknowledged、处理中/In Progress、已解决/Resolved、已关闭/Closed）。
          - 发生时间范围: 日期时间选择器。
          - 告警源/监控对象: 输入框，支持模糊匹配。
          - 规则名称: 下拉选择或输入框。
      - 告警列表: 表格形式展示。关键列包括：选择框、告警级别（可带颜色标识）、告警状态、告警摘要/描述、告警源/关联对象、首次发生时间、最近发生时间、持续时长（若适用）、触发规则名称。操作列包含：查看详情、确认、解决、关闭等（根据告警状态和权限显示）。
      - 默认交互/排序: 默认按发生时间降序、告警级别降序排列。提供分页。新产生的告警或状态变更的告警应有高亮或实时推送提示（若技术支持）。
  - **B. 查看告警详情**: [参考原型 P\_Monitor\_Alarm\_View]
...
| 告警级别        | alarmLevel             | Enum(P0, P1, P2, P3, INFO)                              | 告警的严重程度                          |
...
| (本地)告警状态    | localStatus            | Enum(NEW, ACKNOWLEDGED, IN\_PROGRESS, RESOLVED, CLOSED) | 适配中心内告警的处理状态                     |
