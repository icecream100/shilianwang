...
| 采集数据类型      | dataTypesToCollect       | List\<String>                             | 显示/多选          | 必填，至少选一项。选项基于平台能力预定义。 | 用户选择        | 如：DEVICE\_METADATA, CHANNEL\_METADATA, USER\_ACCESS\_LOGS, VIDEO\_STATS |
| 采集参数        | collectionParams         | JSON/Text                                 | 显示/读写（高级）      | 可选，针对特定数据类型的细化参数      | 用户输入        |                                                                         |
| 采集方式        | collectionMode           | Enum(PERIODIC\_PULL, REALTIME\_SUBSCRIBE) | 显示/选择          | 必填                    | 用户选择        |                                                                         |
